...
#ifdef VERSION_LIB_ENABLE_EXPERIMENTAL_TOSEMVERTOKENS_METHOD2
...
std::vector<VersionLib::VersionToken> VersionLib::toSemVerTokens(std::string version)
{
	...
	short metadataPos = -1;						// Metadata position
	bool foundMetadata = -1;					// Found the metadata info position (-1 is unknown position)
	...
	for (size_t i = 0; i < verStrSize; i++)
	{
		...
		// Detect if is a release separator:
		if (t == '-' && releasePos < 0)
		{
			type = 5;
			releasePos = i;
...
#endif // !VERSION_LIB_ENABLE_EXPERIMENTAL_TOSEMVERTOKENS_METHOD2
