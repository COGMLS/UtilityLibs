...
int main(int argc, const char* argv[])
{
	...
	if (versionTest)
	{
		...
		versionTest->push_back(VersionFormatTest ("7.1.3-rc.1 752"));				// Ok; FAIL ON EXPERIMENTAL FIX: build number is confused with revision.
		versionTest->push_back(VersionFormatTest ("10.3.1-alpha.3 build 569"));		// Ok
		versionTest->push_back(VersionFormatTest ("8.1.93-beta 856"));				// FAIL: Build number is confused with Build type number; FAIL ON EXPERIMENTAL FIX: build number is confused with revision.
		...
		versionTest->push_back(VersionFormatTest ("10-b"));							// Ok (Major and Build type are detected); FAIL ON EXPERIMENTAL FIX: Build type is not detected.
		versionTest->push_back(VersionFormatTest ("10.2-alpha"));					// Ok (Major, Minor and Build type are detected)
		versionTest->push_back(VersionFormatTest ("10.2.8 456"));					// FAIL: Build number is not detected; FAIL ON EXPERIMENTAL FIX: build number is confused with revision.
		versionTest->push_back(VersionFormatTest ("10 487"));						// FAIL: This format is not recognized; FAIL ON EXPERIMENTAL FIX: build number is confused with minor.
		versionTest->push_back(VersionFormatTest ("17.5 782"));						// FAIL: Build type number is confused with patch; FAIL ON EXPERIMENTAL FIX: build number is confused with patch.
		versionTest->push_back(VersionFormatTest ("17.9.5 125"));					// FAIL: Build number is not detected; FAIL ON EXPERIMENTAL FIX: build number is confused with revision.
